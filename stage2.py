...
def main(args):
    ...
    args.go2id = dm.go2id
    args.id2go = {i: go for go, i in args.go2id.items()}
    # model
    ...
    # Alternative: Save only the best checkpoint based on validation loss
    # callbacks.append(plc.ModelCheckpoint(
    #     dirpath="all_checkpoints/" + args.filename + "/",
    #     filename='{epoch:02d}', 
    #     monitor="dataloader0/val loss/dataloader_idx_0",  # Metric to monitor
    #     mode="min",                                       # Lower is better
    #     save_top_k=1,                                     # Keep only the best epoch
    #     every_n_epochs=args.save_every_n_epochs         # Check frequency
    # ))
    ...
    if args.mode == 'train':
        ...
        # Evaluation mode: run validation on test data using existing checkpoint
        ...
        # Set epoch progress to trigger caption generation in validation_step
...
def get_args():
    ...
    parser.add_argument('--head', type=str, default='generation')
